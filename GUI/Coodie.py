...
def sendemail(to, content):
    ...
    server.ehlo()
    server.starttls()
    server.login('akshat.unt@gmail.com', 'yes@akshat') # email id - use any email id whose security/privacy is off
    server.sendmail('unt.akshat@gmail.com', to, content)
    server.close()

def wishme():
    hour = int(datetime.datetime.now().hour)
    if hour >= 0 and hour <= 12:
        var.set("Good Morning akshat") #akshat - your akshat
        window.update()
        speak("Good Morning akshat!")
    elif hour >= 12 and hour <= 18:
        var.set("Good Afternoon akshat!")
        window.update()
        speak("Good Afternoon akshat!")
    else:
        var.set("Good Evening akshat")
        window.update()
        speak("Good Evening akshat!")
    speak("Myself Bot Coodie! How may I help you sir") #Bot name - Give a akshat to your assistant
...
def play():
    ...
    while True:
        ...
        if 'exit' in query:
            ...
            music_dir = 'C:\\User\\Guest 2_2\\Music\\downloads' # Enter the Path of Music Library
